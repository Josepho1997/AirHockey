...
public class PaddleAI extends DynamicGameObject{
	...
	public PaddleAI(float x, float y, float width, float height) {
	...
	public void update(float deltaTime) {
		...
		if(World.puck.velocity.y < 0) {
			...
			if(World.puck.position.y < position.y  && position.y < World.WORLD_HEIGHT - 1) {
		...
		//	Log.d("v", "The puck is moving up");
			...
			if(World.puck.position.y > position.y) {
				velocity.add(0, MOVE_SPEED);
				if(position.y > World.WORLD_HEIGHT - 1) {
		...
		if(position.y + PADDLE_HEIGHT / 2 > World.WORLD_HEIGHT)
			position.y = World.WORLD_HEIGHT - PADDLE_WIDTH / 2;
